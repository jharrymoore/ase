...
cellbounds = CellBounds(bounds={"phi": [30, 150], "chi": [30, 150], "psi": [30, 150]})
...
strainmut = StrainMutation(blmin, stddev=0.7, cellbounds=cellbounds, use_tags=True)
...
operators = OperationSelector(
    [5, 1, 1, 1, 1, 1], [pairing, rattlemut, strainmut, rotmut, rattlerotmut, softmut]
)
...
for step in range(n_to_test):
    ...
    print(
        "Step %d highest raw score in pop: %.3f" % (step, get_raw_score(current_pop[0]))
    )

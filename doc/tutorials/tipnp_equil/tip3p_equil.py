...
md = Langevin(
    atoms, 1 * units.fs, temperature=300 * units.kB, friction=0.01, logfile=tag + ".log"
)
...
atoms.constraints = FixBondLengths(
    [(3 * i + j, 3 * i + (j + 1) % 3) for i in range(len(atoms) / 3) for j in [0, 1, 2]]
)
atoms.calc = TIP3P(rc=7.0)
md = Langevin(
    atoms, 2 * units.fs, temperature=300 * units.kB, friction=0.01, logfile=tag + ".log"
)

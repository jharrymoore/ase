...
class IdealGasThermo(ThermoChem):
    ...
    def get_entropy(self, temperature, pressure, verbose=True):
        ...
        S_t = (2 * np.pi * mass * units._k * temperature / units._hplanck**2) ** (
            3.0 / 2
        )
        ...
        if self.geometry == "monatomic":
            ...
            inertias = (
                self.atoms.get_moments_of_inertia() * units._amu / (10.0**10) ** 2
            )  # kg m^2
...
class CrystalThermo(ThermoChem):
    ...
    def __init__(
        self, phonon_DOS, phonon_energies, formula_units=None, potentialenergy=0.0
    ):

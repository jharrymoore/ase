...
class OFPComparator:
    ...
    def _compare_structure(self, a1, a2):
        ...
        if sorted(fp1) != sorted(fp2):
            raise AssertionError(
                "The two structures have fingerprints " "with different compounds."
            )
        for key in typedic1:
            if not np.array_equal(typedic1[key], typedic2[key]):
                raise AssertionError(
                    "The two structures have a different " "stoichiometry or ordering!"
                )
    ...
    def _get_volume(self, a):
        ...
        if self.dimensions == 3:
            ...
            volume = np.abs(np.dot(np.cross(b0 * v0, b1 * v1), cell[pbc_dir, :]))
    ...
    def _take_fingerprints(self, atoms, individual=False):
        ...
        a.set_pbc(self.pbc)
        nl = NeighborList(
            [self.rcut / 2.0] * len(a), skin=0.0, self_interaction=False, bothways=True
        )
        nl.update(a)

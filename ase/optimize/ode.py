...
def ode12r(
    ...
    if Rn >= maxtol:
        raise OptimizerConvergenceError(
            f"ODE12r: Residual {Rn} is too large " "at iteration 0"
        )
    ...
    for nit in range(1, steps):
        ...
        if extrapolate == 1:  # F(xn + h Fp)
            ...
            raise ValueError(
                f"invalid extrapolate value: {extrapolate}. " "Must be 1, 2 or 3"
            )
        ...
        if accept:
            ...
            if Rn >= maxtol:
                raise OptimizerConvergenceError(
                    f"ODE12r: Residual {Rn} is too " f"large at iteration number {nit}"
                )

            if converged(Fn, X):
                log(f"ODE12r: terminates successfully " f"after {nit} iterations.")
                return X

            # Compute a new step size.
            # Based on the extrapolation and some other heuristics
            h = max(
                0.25 * h, min(4 * h, h_err, h_ls)
            )  # Log steep-size analytic results
        ...
        if abs(h) <= hmin:
            raise OptimizerConvergenceError(
                "ODE12r terminates unsuccessfully" f" Step size {h} too small"
            )

    raise OptimizerConvergenceError(
        f"ODE12r terminates unsuccessfully after " f"{steps} iterations."
    )

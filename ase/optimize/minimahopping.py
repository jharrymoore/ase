...
class MinimaHopping:
    ...
    _default_settings = {
        ...
        "fmax": 0.05,
    }  # eV/A, max force for optimizations
    ...
    def __call__(self, totalsteps=None, maxtemp=None):
        ...
        while True:
            if totalsteps and self._counter >= totalsteps:
                self._log(
                    "msg",
                    "Run terminated. Step #%i reached of "
            ...
            if maxtemp and self._temperature >= maxtemp:
                self._log(
                    "msg",
                    "Run terminated. Temperature is %.2f K;"
                    " max temperature allowed %.2f K." % (self._temperature, maxtemp),
                )
                return
    ...
    def _startup(self):
        ...
        if status == 2.0:
            ...
            if status == 1.0:
                self._log(
                    "msg",
                    "Using existing minima file with %i prior "
    ...
    def _resume(self):
        ...
        if qncount == mdcount:
            ...
            if fmax < self._fmax:
                ...
                self._log(
                    "msg",
                    "qn%05i fmax already less than fmax=%.3f" % (qncount, self._fmax),
                )
                self._counter += 1
                return
            self._optimize()
    ...
    def _check_results(self):
        ...
        if self._previous_energy is None or (
            self._atoms.get_potential_energy() < self._previous_energy + self._Ediff
        ):
            self._log("msg", "Accepted new minimum.")
            ...
            self._log(
                "msg", "Rejected new minimum due to energy. " "Restoring last minimum."
            )
            self._atoms.positions = self._previous_optimum.positions
    ...
    def _molecular_dynamics(self, resume=None):
        ...
        if resume:
            ...
            if os.path.getsize("md%05i.traj" % resume) == 0:
                self._log(
                    "msg",
                    "md%05i.traj is empty. Resuming from "
        ...
        if not thermalized:
            MaxwellBoltzmannDistribution(
                self._atoms, temperature_K=self._temperature, force_temp=True
            )
...
class MHPlot:
    ...
    def _makecanvas(self):
        ...
        ratio = 2.0  # size of an energy plot to a parameter plot
        figwidth = 1.0 - lm - rm
        ...
        ediffax = fig.add_axes(
            (lm, bm + 2.0 * epotheight + vg1 + vg2, figwidth, parfigheight)
        )
        tempax = fig.add_axes(
            (
                lm,
                (bm + 2 * epotheight + vg1 + 2 * vg2 + parfigheight),
                figwidth,
                parfigheight,
            )
        )
    ...
    def _plot_energy(self, step, line):
        ...
        if status == "accepted":
            ...
            self._ax.text(
                step + 0.51, energy, r"$\hookleftarrow$", color="red", va="center"
            )
        elif status == "previous minimum":
            self._ax.text(
                step + 0.51, energy, r"$\leftarrow$", color="red", va="center"
            )

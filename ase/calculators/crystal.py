...
class CRYSTAL(FileIOCalculator):
    ...
    def read_results(self):
        ...
        with open(
            os.path.join(self.directory, "OUTPUT"), "rt", encoding="latin-1"
        ) as myfile:
        ...
        for n, line in enumerate(self.lines):
            if "TOTAL ATOMIC CHARGE" in line:
                chargestart = n + 1
        lines1 = self.lines[
            chargestart : (chargestart + (len(self.atoms) - 1) // 6 + 1)
        ]
        ...
        for n, line in enumerate(self.lines):
            if "DIPOLE MOMENT ALONG" in line:
                dipolestart = n + 2
                dipole = np.array(
                    [float(f) for f in self.lines[dipolestart].split()[2:5]]
                )
                break
        # debye to e*Ang
...
class PointChargePotential:
    ...
    def read_forces_on_pointcharges(self):
        ...
        for line in lines1:
            external_forces.append([float(i) for i in line.split()[2:]])
    ...
    def read_pc_corrections(self):
        ...
        e = [float(x.split()[-1]) for x in lines if "SELF-INTERACTION ENERGY(AU)" in x][
            0
        ]

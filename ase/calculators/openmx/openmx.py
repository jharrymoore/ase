...
class OpenMX(FileIOCalculator):
    ...
    default_pbs = {"processes": 1, "walltime": "10:00:00", "threads": 1, "nodes": 1}
        ...
        if pbs:
            ...
            for key in pbs:
                if key not in self.default_pbs:
                    ...
                    raise TypeError(
                        'Unexpected keyword "{0}" in "pbs" '
                        "dictionary.  Must be one of: {1}".format(key, allowed)
                    )
    ...
    def run_pbs(self, prefix="test"):
        ...
        bashArgs = "#!/bin/bash \n cd $PBS_O_WORKDIR\n"
        jobName = prefix
        cmd = bashArgs + "mpirun -hostfile $PBS_NODEFILE openmx %s > %s" % (
            inputfile,
            outfile,
        )
        echoArgs = ["echo", "$' %s'" % cmd]
        ...
        out = subprocess.Popen(
            wholeCmd, shell=True, stdout=subprocess.PIPE, universal_newlines=True
        )
        out = out.communicate()[0]
        jobNum = int(re.match(r"(\d+)", out.split()[0]).group(1))

        self.prind(
            "Queue number is " + str(jobNum) + "\nWaiting for the Queue to start"
        )
        ...
        self.print_file(
            file=outfile, running=isRunning, jobNum=jobNum, status="R", qstat="qstat"
        )
    ...
    def set(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key not in self.default_parameters.keys():
                raise KeyError('Unkown keyword "%s" and value "%s".' % (key, value))
            ...
            if key in ["dat_arguments"] and isinstance(value, dict):
                ...
                for subkey in kwargs[key]:
                    if subkey not in default_dict:
                        ...
                        raise TypeError(
                            'Unknown subkeyword "{0}" of keyword '
                            '"{1}".  Must be one of: {2}'.format(subkey, key, allowed)
                        )
        ...
        if value is not None and not (isinstance(value, (float, int)) and value > 0):
            mess = (
                "'%s' must be a positive number(in eV), \
                got '%s'"
                % ("energy_cutoff", value)
            )
        ...
        try:
            ...
            if self["scf_spinpolarization"] is None:
                ...
                if self["scf_spinpolarization"] == "on":
                    ...
                    or np.any(self["initial_magnetic_moments_euler_angles"]) is not None
                ):
                    return 1
        except KeyError:
            return 1

...
def parse_siesta_version(output: bytes) -> str:
    ...
    if match is None:
        raise RuntimeError(
            "Could not get Siesta version info from output " "{!r}".format(output)
        )
...
class Siesta(FileIOCalculator):
    ...
    allowed_spins = ["non-polarized", "collinear", "non-collinear", "spin-orbit"]
    allowed_xc = {
        "LDA": ["PZ", "CA", "PW92"],
        "GGA": [
            "PW91",
    ...
    def __init__(self, command=None, **kwargs):
        ...
        # Call the base class.
        FileIOCalculator.__init__(self, command=command, **parameters)
    ...
    def species(self, atoms):
        ...
        default_species = [
            s for s in species if (s["tag"] is None) and s["symbol"] in symbols
        ]
    ...
    def set(self, **kwargs):
        ...
        if "spin" in kwargs:
            if kwargs["spin"] == "UNPOLARIZED":
                warnings.warn(
                    "The keyword 'UNPOLARIZED' is deprecated,"
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        with open(filename, "w") as fd:
            ...
            if self["spin"] == "collinear":
                fd.write(
                    format_fdf("SpinPolarized", (True, "# Backwards compatibility."))
                )
            elif self["spin"] == "non-collinear":
                fd.write(
                    format_fdf("NonCollinearSpin", (True, "# Backwards compatibility."))
                )
        ...
        for c in a.constraints:
            if isinstance(c, FixAtoms):
                ...
                if (max(norm_dir) - 1.0) > 1e-6:
                    raise RuntimeError(
                        "norm_dir: {} -- must be one of the Cartesian axes...".format(
                            norm_dir
                        )
                    )
                ...
                warnings.warn(
                    "Constraint {} is ignored at {}".format(
                        str(c), sys._getframe().f_code
                    )
                )
        return a2c
    ...
    def read_results(self):
        ...
        if self.results["hsx"] is not None:
            self.read_pld(self.results["hsx"].norbitals, len(self.atoms))

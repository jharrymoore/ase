...
class Onetep(FileIOCalculator):
    ...
    def read(self, label):
        ...
        if not (read_lattice and read_species and read_positions):
            raise ReadError(
                "Failed to read in essential calculation"
    ...
    def _read_excitations(self, out):
        ...
        while line:
            ...
            if len(words) == 0:
                break
            excitations.append(
                [float(words[0]), float(words[1]) * Hartree, float(words[2])]
            )
            line = out.readline()
    ...
    def _write_dat(self, force_write=True):
        ...
        if (
            "xc" in parameters
            and "xc_functional" in parameters
            and parameters["xc"] != parameters["xc_functional"]
        ):
            raise Exception(
                "Conflicting functionals defined! %s vs. %s"
        ...
        pos_block = [
            ("%s %8.6f %8.6f %8.6f" % (x + z, y[0], y[1], y[2]))
            for (x, y, z) in zip(atoms.get_chemical_symbols(), positions, tags)
        ]
        ...
        if (self.parameters["ngwf_radius_cond"] > 0) or len(self.species_cond) == len(
            self.species
        ):
        ...
        if (self.parameters["ngwf_radius_cond"] > 0) or len(self.solvers_cond) == len(
            self.species
        ):

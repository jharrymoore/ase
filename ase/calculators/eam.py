    ...
    * There is a bug in early versions of scipy that will cause eam.py to
    ...
    def _read_potential(self, fd):
        ...
        if self.form == "eam":  # single element eam file (aka funcfl)
            ...
            self.embedded_data = np.zeros([self.Nelements, self.nrho])
            self.density_data = np.zeros([self.Nelements, self.Nelements, self.nr])
            self.Z = np.zeros([self.Nelements], dtype=int)
            self.mass = np.zeros([self.Nelements])
            self.a = np.zeros([self.Nelements])
            self.lattice = []
            ...
            for elem in range(self.Nelements):
                ...
                self.density_data[elem, :, :] = np.float_(
                    data[d : (d + self.nr * self.Nelements)]
                ).reshape([self.Nelements, self.nr])
    ...
    def set_splines(self):
        ...
        for i in range(self.Nelements):
            for j in range(i, self.Nelements):
                self.phi[i, j] = spline(
                    self.r[1:], self.rphi_data[i, j][1:] / self.r[1:], k=3
                )
    ...
    def _write_potential(self, fd, nc, numformat):
        ...
        for i in range(self.Nelements):
            ...
            np.savetxt(
                fd,
                self.embedded_energy[i](rhos).reshape(self.nrho // nc, nc),
                fmt=nc * [numformat],
            )
            if self.form == "fs":
                for j in range(self.Nelements):
                    np.savetxt(
                        fd,
                        self.electron_density[i, j](rs).reshape(self.nr // nc, nc),
                        fmt=nc * [numformat],
                    )
            else:
                np.savetxt(
    ...
    def update(self, atoms):
        ...
        if np.any(unavailable):
            raise RuntimeError(
                "These elements are not in the potential: %s" % elements[unavailable]
            )
        ...
        self.neighbors = NeighborList(
            cutoffs, skin=self.parameters.skin, self_interaction=False, bothways=True
        )
    ...
    def calculate_energy(self, atoms):
        ...
        for i in range(len(atoms)):  # this is the atom to be embedded
            ...
            for j_index in range(self.Nelements):
                ...
                if not use.any():
                    continue
                pair_energy += (
                    np.sum(self.phi[self.index[i], j_index](r[nearest][use])) / 2.0
                )
        ...
        if self.form == "adp":
            ...
            adp_result = dict(
                adp_mu=mu_energy, adp_lam=lam_energy, adp_trace=trace_energy
            )
        ...
        for gamma in range(3):
            ...
            for alpha in range(3):
                for beta in range(3):
                    ...
                    term4 += (
                        (lam_i[alpha, beta] + lam[:, alpha, beta])
                        * self.d_q[form1][form2](r)
                        * rs
                    ) / r
    ...
    def plot(self, name=""):
        ...
        self.elem_subplot(
            rho,
            self.embedded_energy,
            r"$\rho$",
            r"Embedding Energy $F(\bar\rho)$",
            name,
            plt,
        )
        ...
        self.multielem_subplot(
            r, self.phi, r"$r$", r"Pair Potential $\phi(r)$", name, plt
        )

...
class DMol3(FileIOCalculator):
    ...
    def read_results(self):
        ...
        if not finished:
            raise RuntimeError(
                "DMol3 run failed, see outmol file for" " more info\n\n%s" % message
            )
    ...
    def find_dmol_transformation(self, tol=1e-4):
        ...
        if np.all(self.atoms.pbc):  # [True, True, True]
            ...
            if (np.linalg.norm(self.atoms.positions - dmol_atoms.positions) < tol) and (
                np.linalg.norm(self.atoms.cell - dmol_atoms.cell) < tol
            ):
                self.internal_transformation = False
            else:
                ...
                if abs(np.linalg.det(R) - 1.0) > tol:
                    raise RuntimeError(
                        "Error: transformation matrix does" " not have determinant 1.0"
                    )
                if err < tol:
                    ...
                    raise RuntimeError(
                        "Error: Could not find dmol" " coordinate transformation"
                    )
            ...
            if line.startswith("Energy components"):
                ...
                while not lines[m].strip() == "":
                    energies[lines[m].split("=")[0].strip()] = (
                        float(re.findall(r"[-+]?\d*\.\d+|\d+", lines[m])[0]) * Hartree
                    )
                    m += 1
        return energies

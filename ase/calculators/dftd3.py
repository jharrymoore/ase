...
import numpy as np
from ase.calculators.calculator import BaseCalculator, FileIOCalculator, Calculator
...
class PureDFTD3(FileIOCalculator):
    ...
    def set(self, **kwargs):
        ...
        if self.parameters["cnthr"] > self.parameters["cutoff"]:
            warn(
                ...
                "to {cutoff}."
                "".format(
                    cnthr=self.parameters["cnthr"], cutoff=self.parameters["cutoff"]
                )
            )
        ...
        if damppars:
            ...
            if missing_damppars and missing_damppars != valid_damppars:
                raise ValueError(
                    ...
                    "".format(damping, ", ".join(valid_damppars), ", ".join(damppars))
                )
    ...
    def _read_and_broadcast_results(self):
        ...
        if self.comm.rank == 0:
            output = DFTD3Output(directory=self.directory, stdout_path=self._outname())
            dct = output.read(
...
class DFTD3Output:
    ...
    def parse_energy(self, fd, outname):
        ...
        raise RuntimeError(
            "Could not parse energy from dftd3 " "output, see file {}".format(outname)
        )

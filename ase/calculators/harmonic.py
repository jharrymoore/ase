...
class HarmonicForceField:
    def __init__(
        ...
        **q**, e.g. when the transformation between the coordinate
        ...
        """
        self.check_input([get_q_from_x, get_jacobian], variable_orientation, cartesian)
        ...
        self.zero_eigvals = len(
            np.flatnonzero(np.abs(self.Gmat_eigvals) < self.parameters["zero_thresh"])
        )
    ...
    def transform2reference_hessians(self, jac0, ijac0):
        ...
        w[
            (0.0 < w)
            & (  # substitute small eigenvalues by lower limit

...
class DiffusionCoefficient:
    def __init__(self, traj, timestep, atom_indices=None, molecule=False):
        ...
        if self.is_molecule:
            ...
            self.no_of_atoms = [
                traj[0].get_chemical_symbols().count(symbol)
                for symbol in self.types_of_atoms
            ]
        ...
        for segment_no in range(self.no_of_segments):
            ...
            for image_no in range(0, len(seg)):
                ...
                # displacement
                    ...
                    com_disp = np.zeros(3)
                    for atom_no in self.atom_indices:
                        com_disp += seg[image_no].positions[atom_no] / len(
                            self.atom_indices
                        )
                ...
                for sym_index in range(self.no_of_types_of_atoms):
                    ...
                    for xyz in range(3):
                        self.xyz_segment_ensemble_average[segment_no][sym_index][xyz][
                            image_no
    ...
    def _fit_data(self, x, y):
        ...
        for xyz in range(3):
            slopes[xyz], intercepts[xyz] = np.linalg.lstsq(x_edited, y[xyz], rcond=-1)[
                0
            ]
    ...
    def get_diffusion_coefficients(self):
        ...
        slopes = [
            np.mean(self.slopes[sym_index])
            for sym_index in range(self.no_of_types_of_atoms)
        ]
        std = [
            np.std(self.slopes[sym_index])
            for sym_index in range(self.no_of_types_of_atoms)
        ]
    ...
    def plot(self, ax=None, show=False):
        ...
        for segment_no in range(self.no_of_segments):
            ...
            for sym_index in range(self.no_of_types_of_atoms):
                ...
                line = np.mean(
                    self.slopes[sym_index][segment_no]
                ) * fs_conversion * graph_timesteps[start:end] + np.mean(
                    self.intercepts[sym_index][segment_no]
                )
                if segment_no == 0:
            ...
            ax.plot(
                [x_coord, x_coord],
                [-0.001, 1.05 * np.amax(self.xyz_segment_ensemble_average)],
                color="grey",
                linestyle=":",
            )
    ...
    def print_data(self):
        ...
        for sym_index in range(self.no_of_types_of_atoms):
            ...
            for segment_no in range(self.no_of_segments):
                print(
                    ...
                    % (
                        segment_no,
                        np.mean(self.slopes[sym_index][segment_no]) * fs_conversion,
        ...
        for sym_index in range(self.no_of_types_of_atoms):
            print(
                ...
                % (
                    self.types_of_atoms[sym_index],
                    slopes[sym_index] * fs_conversion,
                    std[sym_index] * fs_conversion,
                )
            )
        print("---")

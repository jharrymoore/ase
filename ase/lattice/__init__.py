...
special_points={GNPSS1XYY1Z}, kpts=[51x3])
    ...
    def description(self) -> str:
        ...
        coordstring = "\n".join(
            [
                "    {:2s} {:7.4f} {:7.4f} {:7.4f}".format(label, *points[label])
                for label in labels
            ]
        )
...
{special_points}
""".format(
            repr=str(self), variant=self._variant, special_points=coordstring
        )
        return string
...
"""
    ...
    def __init__(self, name, special_point_names, special_path, special_points=None):
        self.name = name
...
@bravaisclass(
    ...
    "oP",
    "abc",
    [["ORC", "GRSTUXYZ", "GXSYGZURTZ,YT,UX,SR", sc_special_points["orthorhombic"]]],
)
...
@bravaisclass(
    ...
    [
        ...
        ["MCLC5", "GFF1F2HH1H2II1LMNN1XYY1Y2Y3Z", "GYFLI,I1ZHF1,H1Y1XGN,MG", None],
    ],
)
...
class OBL(BravaisLattice):
    def __init__(self, a, b, alpha, **kwargs):
        check_rect(a, b)
        if alpha >= 90:
            raise UnconventionalLattice(f"Expected alpha < 90, got alpha={alpha}")
...
@bravaisclass(
    ...
    "tp",
    ("a",),
    [["SQR", "GMX", "MGXM", get_subset_points("GMX", sc_special_points["tetragonal"])]],
    ndim=2,
)

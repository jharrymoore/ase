...
class WannierSpec:
    ...
    def random(self, rng, edf_k):
        ...
        for U, M, L in zip(U_kww, self.fixedstates_k, edf_k):
            ...
            if L > 0:
                C_kul.append(
                    random_orthogonal_matrix(self.Nb - M, rng=rng, real=False)[:, :L]
                )
            else:
                C_kul.append(_empty())
        return WannierState(C_kul, U_kww)
    ...
    def initial_wannier(self, calc, method, kptgrid, edf_k, spin):
        C_kul, U_kww = calc.initial_wannier(
            method, kptgrid, self.fixedstates_k, edf_k, spin, self.Nb
        )
        return WannierState(C_kul, U_kww)
    ...
    def scdm(self, calc, kpt_kc, spin):
        ...
        ps = calc.get_pseudo_wave_function(band=self.Nw, kpt=0, spin=0)
        Ng = ps.size
        pseudo_nkG = np.zeros((self.Nb, self.Nk, Ng), dtype=np.complex128)
        for k in range(self.Nk):
            for n in range(self.Nb):
                pseudo_nkG[n, k] = calc.get_pseudo_wave_function(
                    band=n, kpt=k, spin=spin
                ).ravel()

        # Use initial guess to determine U and C
        C_kul, U_kww = scdm(
            pseudo_nkG, kpts=kpt_kc, fixed_k=self.fixedstates_k, Nw=self.Nw
        )
        return WannierState(C_kul, U_kww)

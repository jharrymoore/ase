...
def read_cfg(fd):
    ...
    if current_atom != nat:
        raise RuntimeError(
            "Number of atoms reported for CFG file (={0}) and "
            "number of atoms actually read (={1}) differ.".format(nat, current_atom)
        )
    ...
    if vels is None:
        a = ase.Atoms(
            symbols=syms, masses=masses, scaled_positions=spos, cell=cell, pbc=True
        )
    else:
        a = ase.Atoms(
            symbols=syms,

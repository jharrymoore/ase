...
def read_wout_all(fileobj: IO[str]) -> Dict[str, Any]:
    ...
    cell = [[float(x) for x in line.split()[-3:]] for line in lines[n + 1 : n + 4]]
    ...
    while n > 0:
        ...
        return {"atoms": atoms, "centers": np.zeros((0, 3)), "spreads": np.zeros((0,))}
    ...
    return {"atoms": atoms, "centers": np.array(centers), "spreads": np.array(spreads)}


def read_wout(fileobj: IO[str], include_wannier_function_centers: bool = True) -> Atoms:

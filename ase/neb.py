...
class NEBState:
    ...
    def eqlength(self):
        images = self.images
        beeline = (
            images[self.neb.nimages - 1].get_positions() - images[0].get_positions()
        )
...
class ImprovedTangentMethod(NEBMethod):
    ...
    def get_tangent(self, state, spring1, spring2, i):
        ...
        if energies[i + 1] > energies[i] > energies[i - 1]:
            ...
            deltavmax = max(
                abs(energies[i + 1] - energies[i]), abs(energies[i - 1] - energies[i])
            )
            deltavmin = min(
                abs(energies[i + 1] - energies[i]), abs(energies[i - 1] - energies[i])
            )
...
class ASENEBMethod(NEBMethod):
    ...
    def add_image_force(
        ...
        imgforce -= (
            np.vdot(spring1.t * spring1.k - spring2.t * spring2.k, tangent) * factor
        )
...
class BaseNEB:
    ...
    def get_forces(self):
        ...
        if not self.parallel:
            ...
            threads = [
                threading.Thread(
                    target=run, args=(images[i], energies[i : i + 1], forces[i - 1 : i])
                )
                for i in range(1, self.nimages - 1)
            ]
    ...
    def set_calculators(self, calculators):
        ...
        if n == self.nimages:
            ...
            raise RuntimeError(
                "len(calculators)=%d does not fit to len(images)=%d" % (n, self.nimages)
            )
    ...
    for i in range(1, len(images) - 1):
        ...
        if use_scaled_coord:
            ...
            if apply_constraint is None:
                ...
                unconstrained_image.set_positions(new_pos, apply_constraint=False)
...
def fit0(*args, **kwargs):
    raise DeprecationWarning(
        "fit0 is deprecated. Use `fit_raw` from " "`ase.utils.forcecurve` instead."
    )

...
def calculate_band_structure(
    atoms, path=None, scf_kwargs=None, bs_kwargs=None, kpts_tol=1e-6, cell_tol=1e-6
):
...
def get_band_structure(atoms=None, calc=None, path=None, reference=None):
    ...
    for s in range(calc.get_number_of_spins()):
        energies.append([calc.get_eigenvalues(kpt=k, spin=s) for k in range(len(kpts))])
    energies = np.array(energies)

    if path is None:
        from ase.dft.kpoints import BandPath, resolve_custom_points, find_bandpath_kinks
        ...
        path = BandPath(
            standard_path.cell, kpts=kpts, path=pathspec, special_points=special_points
        )

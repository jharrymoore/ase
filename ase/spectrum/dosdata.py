...
class DOSData(metaclass=ABCMeta):
    ...
    @staticmethod
    def _delta(
        x: np.ndarray, x0: float, width: float, smearing: str = "Gauss"
    ) -> np.ndarray:
    ...
    def plot(
        ...
        return self.sample_grid(
            ...
            ax=ax, xmin=xmin, xmax=xmax, show=show, filename=filename, mplargs=mplargs
        )
...
class GeneralDOSData(DOSData):
    ...
    def __init__(self, energies: Floats, weights: Floats, info: Info = None) -> None:
    ...
    def copy(self: D) -> D:  # noqa F821
        return type(self)(
            self.get_energies(), self.get_weights(), info=self.info.copy()
        )
    ...
    *intersection* of the inputs: only key-value pairs that were common to both
    ...
    def __add__(self, other: "RawDOSData") -> "RawDOSData":
        if not isinstance(other, RawDOSData):
            raise TypeError(
                "RawDOSData can only be combined with other " "RawDOSData objects"
            )
...
class GridDOSData(GeneralDOSData):
    ...
    def _sample(
        ...
        return (
            super()._sample(energies=energies, width=width, smearing=smearing)
            * current_spacing
        )
    ...
    def __add__(self, other: "GridDOSData") -> "GridDOSData":
        ...
        if not isinstance(other, GridDOSData):
            raise TypeError(
                "GridDOSData can only be combined with other " "GridDOSData objects"
            )
        if len(self._data[0, :]) != len(other.get_energies()):
            raise ValueError(
                "Cannot add GridDOSData objects with different-" "length energy grids."
            )

        if not np.allclose(self._data[0, :], other.get_energies()):
            raise ValueError(
                "Cannot add GridDOSData objects with different " "energy grids."
            )

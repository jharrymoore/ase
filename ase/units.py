...
def create_units(codata_version):
    ...
    try:
        ...
        raise NotImplementedError(
            'CODATA version "{0}" not implemented'.format(codata_version)
        )
    ...
    u["nm"] = 10.0
    u["Bohr"] = (
        4e10 * pi * u["_eps0"] * u["_hbar"] ** 2 / u["_me"] / u["_e"] ** 2
    )  # Bohr radius
    ...
    u["alpha"] = (
        u["_e"] ** 2 / (4 * pi * u["_eps0"]) / u["_hbar"] / u["_c"]
    )  # fine structure constant
    u["invcm"] = 100 * u["_c"] * u["_hplanck"] / u["_e"]  # cm^-1 energy unit

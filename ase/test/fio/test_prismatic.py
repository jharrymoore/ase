...
def test_write_read_cycle_xyz_prismatic():
    ...
    filename = "SI100.XYZ"
    atoms.write(
        filename=filename, format="prismatic", comments="one unit cell of 100 silicon"
    )
    ...
    np.testing.assert_allclose(
        atoms.get_array("occupancies"), atoms_loaded.get_array("occupancies")
    )
    np.testing.assert_allclose(
        atoms.get_array("debye_waller_factors"),
...
def test_write_error():
    ...
    with pytest.raises(ValueError):
        # DW missing keys
        atoms_STO.write(
            filename, format="prismatic", debye_waller_factors=STO_DW_dict_Ti_missing
        )

...
class TestDOSCollection:
    ...
    def another_rawdos(self):
        return RawDOSData(
            [3.0, 2.0, 5.0], [1.0, 0.0, 2.0], info={"other_key": "other_value"}
        )
    ...
    def test_collection_equality(self, rawdos, another_rawdos):
        equality_data = [
            ([], [], True),
            ([rawdos], [rawdos], True),
            ([rawdos, another_rawdos], [rawdos, another_rawdos], True),
            ([], [rawdos], False),
            ([rawdos], [], False),
            ([rawdos, another_rawdos], [rawdos], False),
            ([rawdos, another_rawdos], [another_rawdos, rawdos], False),
        ]

        for series_1, series_2, isequal in equality_data:
            assert (
                MinimalDOSCollection(series_1)._almost_equals(
                    MinimalDOSCollection(series_2)
                )
                == isequal
            )
    ...
    def test_addition(self, rawdos, another_rawdos):
        ...
        assert (dc + MinimalDOSCollection([another_rawdos]))._almost_equals(
            dc + another_rawdos
        )
    ...
    @pytest.mark.parametrize(
        "options",
        [
            {"energies": [1.0, 1.1, 1.2], "width": 1.3, "smearing": "Gauss"},
            {"energies": [1.7, 2.1, 2.0], "width": 3.4, "smearing": "Gauss"},
        ],
    )
    ...
    sample_grid_options = [
        {"npts": 10, "xmin": -2, "xmax": 10, "padding": 3, "width": 1},
        {"npts": 12, "xmin": 0, "xmax": 4, "padding": 2.1, "width": 2.3},
    ]
    ...
    def test_sample_grid(self, rawdos, another_rawdos, options):
        ...
        dc = MinimalDOSCollection([rawdos, another_rawdos])
        dos = dc.sample_grid(
            10, xmax=options["xmax"], padding=options["padding"], width=options["width"]
        )
        energies = dos.get_energies()

        assert (
            pytest.approx(energies[0])
        ...
        # Check auto maximum
        dos = dc.sample_grid(
            10, xmin=options["xmin"], padding=options["padding"], width=options["width"]
        )
        energies = dos.get_energies()
        assert pytest.approx(energies[0]) == options["xmin"]
        assert (
            pytest.approx(energies[-1])
    ...
    @pytest.mark.parametrize(
        "x, weights, bad_info",
        [
            ([1, 2, 4, 5], [[0, 1, 1, 0], [2, 1, 2, 1]], [{"notenough": "entries"}]),
            ([3.1, 2.4, 1.1], [[2, 1.0, 3.12]], [{"too": "many"}, {"entries": "here"}]),
        ],
    )
    ...
    collection_data = [
        ...
        [([1.0, 2.0, 3.0], [1.0, 1.0, 2.0]), ([2.0, 3.5], [0.5, 1.0])],
        [([1.0, 2.0, 3.0], [1.0, 1.0, 2.0]), ([2.0, 3.5], [0.5, 1.0]), ([1.0], [0.25])],
    ]
    collection_info = [
        [{"el": "C", "index": "1"}],
        [{"el": "C", "index": "1"}, {"el": "C", "index": "2"}],
        [
            {"el": "C", "index": "1"},
            {"el": "C", "index": "2"},
            {"el": "C", "index": "2"},
    ...
    expected_sum = [
        ([1.0, 2.0, 3.0], [1.0, 1.0, 2.0], {"el": "C", "index": "1"}),
        ([1.0, 2.0, 3.0, 2.0, 3.5], [1.0, 1.0, 2.0, 0.5, 1.0], {"el": "C"}),
        ([1.0, 2.0, 3.0, 2.0, 3.5, 1.0], [1.0, 1.0, 2.0, 0.5, 1.0, 0.25], {"el": "C"}),
    ]

    @pytest.mark.parametrize(
        "collection_data, collection_info, expected",
        zip(collection_data, collection_info, expected_sum),
    )
    ...
    @pytest.mark.parametrize(
        "collection_data, collection_info", zip(collection_data, collection_info)
    )
    ...
    @pytest.mark.parametrize(
        "select_info, select_query, " "select_result, select_not_result, sum_by_result",
        zip(select_info, select_query, select_result, select_not_result, sum_by_result),
    )
    def test_select(
        self, select_info, select_query, select_result, select_not_result, sum_by_result
    ):
        dc = DOSCollection(
            [RawDOSData([0.0], [0.0], info=info) for info in select_info]
        )
...
class TestGridDOSCollection:
    ...
    def test_select(self, griddos, another_griddos):
        ...
        assert gdc.select(my_key="my_value")._almost_equals(
        ...
        assert gdc.select_not(my_key="my_value")._almost_equals(
            GridDOSCollection([another_griddos])
        )
        assert (
            gdc.select(my_key="my_value")
            .select_not(my_key="my_value")
            ._almost_equals(GridDOSCollection([], energies=griddos.get_energies()))
        )

    def test_sequence(self, griddos, another_griddos):
        gdc = GridDOSCollection([griddos, another_griddos])

        for i, (coll_dosdata, dosdata) in enumerate(
            zip(gdc, [griddos, another_griddos])
        ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                np.linspace(1, 10, 12),
                [np.linspace(4, 1, 12), np.sin(range(12))],
                [{"entry": "1"}, {"entry": "2"}],
                None,
            ),
    ...
    def test_plot_no_resample(self, griddoscollection, figure):
        ...
        assert np.allclose(
            ax.get_lines()[0].get_xdata(), griddoscollection[0].get_energies()
        )
        assert np.allclose(
            ax.get_lines()[1].get_ydata(), griddoscollection[1].get_weights()
        )

...
class TestRawDosData:
    ...
    equality_data = [
        (
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "H"}),
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "H"}),
            True,
        ),
        (
            ((1.0, 3.0), (3.0, 4.0), {"symbol": "H"}),
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "H"}),
            False,
        ),
        (
            ((1.0, 2.0), (3.0, 5.0), {"symbol": "H"}),
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "H"}),
            False,
        ),
        (
            ((1.0, 3.0), (3.0, 5.0), {"symbol": "H"}),
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "H"}),
            False,
        ),
        (
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "H"}),
            ((1.0, 2.0), (3.0, 4.0), {"symbol": "He"}),
            False,
        ),
        (
            ((1.0, 3.0), (3.0, 4.0), {"symbol": "H"}),
    ...
    def test_equality(self, data_1, data_2, isequal):
        assert (
            RawDOSData(*data_1[:2], info=data_1[2])._almost_equals(
                RawDOSData(*data_2[:2], info=data_2[2])
            )
        ) == isequal
    ...
    def test_addition(self, sparse_dos, another_sparse_dos):
        ...
        assert np.allclose(
            summed_dos.get_energies(), [1.2, 3.4, 5.0, 8.0, 2.0, 2.0, 5.0]
        )
        assert np.allclose(
            summed_dos.get_weights(), [3.0, 2.1, 0.0, 1.0, 1.0, 1.0, 1.0]
        )

    sampling_data_args_results = [
        # Special case: peak max at width 1
        ([[0.0], [1.0]], [[0.0], {"width": 1}], [1.0 / (np.sqrt(2.0 * np.pi))]),
        # Peak max with different width, position
        ([[1.0], [2.0]], [[1.0], {"width": 0.5}], [2.0 / (np.sqrt(2.0 * np.pi) * 0.5)]),
    ...
    def test_plot_deltas(self, sparse_dos, figure, linewidth):
        ...
        assert ax_out == ax
        assert np.allclose(
            list(map(lambda x: x.vertices, ax.get_children()[0].get_paths())),
            [
                [[1.2, 0.0], [1.2, 3.0]],
                [[3.4, 0.0], [3.4, 2.1]],
                [[5.0, 0.0], [5.0, 0.0]],
...
class TestGridDosData:
    ...
    def test_addition(self, dense_dos, another_dense_dos):
        ...
        with pytest.raises(ValueError):
            dense_dos + GridDOSData(
                dense_dos.get_energies() + 1.0, dense_dos.get_weights()
            )
        with pytest.raises(ValueError):
            dense_dos + GridDOSData(
                dense_dos.get_energies()[1:], dense_dos.get_weights()[1:]
            )
    ...
    def test_resampling_consistency(self, dense_dos, denser_dos):
        ...
        assert np.allclose(
            from_dense.get_weights(), from_denser.get_weights(), rtol=0.05, atol=0.01
        )
    ...
    smearing_args = [
        (dict(npts=0, width=None), (0, None)),
        (dict(npts=10, width=None, default_width=5.0), (10, 5.0)),

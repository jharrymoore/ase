...
from ase.ga.utilities import closest_distances_generator, atoms_too_close, CellBounds
...
def test_bulk_operators(seed, tmp_path):
    ...
    volume = 40.0 * sum([x[1] for x in blocks])  # cell volume in angstrom^3
    ...
    blmin = closest_distances_generator(
        atom_numbers=atom_numbers, ratio_of_covalent_radii=1.3
    )
    ...
    softmut = SoftMutation(
        blmin, bounds=[2.0, 5.0], used_modes_file=None, use_tags=True
    )  # no rng
    rotmut = RotationalMutation(blmin, fraction=0.3, min_angle=0.5 * np.pi, rng=rng)
    ...
    mutations = [strainmut, softmut, rotmut, rattlemut, rattlerotmut, permut, combmut]
    ...
    softmut_with = SoftMutation(
        blmin, bounds=[2.0, 5.0], use_tags=True, used_modes_file=modes_file
    )  # no rng
    no_muts = 3

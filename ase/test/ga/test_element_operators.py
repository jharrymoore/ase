def test_element_operators(seed):
    ...
    cations = ["Sr", "Ba", "Ca", "Mg"]
    anions = ["Cl", "F", "Br"]
    op = OnePointElementCrossover([cations, anions], [3, 2], [0.25, 0.5], rng=rng)
    ...
    op = RandomElementMutation([cations, anions], [3, 2], [0.25, 0.5], rng=rng)
    ...
    op = RandomElementMutation(anions, 2, 0.5, rng=rng)
    ...
    a1.info["confid"] = 1
    op = MoveDownMutation(cations, 2, 0.5, rng=rng)
    ...
    op = MoveUpMutation(cations, 1, 1.0, rng=rng)
    ...
    a1 = Atoms("NiNiBrBr")
    a1.info["confid"] = 1
    op = MoveRightMutation(cations, 1, 1.0, rng=rng)
    a2, desc = op.get_new_individual([a1])
    a2.info["confid"] = 2
    syms = a2.get_chemical_symbols()
    ...
    op = MoveLeftMutation(cations, 2, 0.5, rng=rng)
    a3, desc = op.get_new_individual([a2])
    syms = a3.get_chemical_symbols()

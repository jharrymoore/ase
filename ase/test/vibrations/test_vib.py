...
def n2_data():
    return {
        "atoms": Atoms(
            "N2", positions=[[0.0, 0.0, 0.05095057], [0.0, 0.0, 1.04904943]]
        ),
        "hessian": np.array(
            [
                [
                    [[4.67554672e-03, 0.0, 0.0], [-4.67554672e-03, 0.0, 0.0]],
                    [[0.0, 4.67554672e-03, 0.0], [0.0, -4.67554672e-03, 0.0]],
                    [[0.0, 0.0, 3.90392599e01], [0.0, 0.0, -3.90392599e01]],
                ],
                [
                    [[-4.67554672e-03, 0.0, 0.0], [4.67554672e-03, 0.0, 0.0]],
                    [[0.0, -4.67554672e-03, 0.0], [0.0, 4.67554672e-03, 0.0]],
        ...
        "ref_forces": np.array([[0.0, 0.0, -2.26722e-1], [0.0, 0.0, 2.26722e-1]]),
    }
...
def n2_unstable_data():
    return {
        "atoms": Atoms("N2", positions=[[0.0, 0.0, 0.45], [0.0, 0.0, -0.45]]),
        "hessian": np.array(
            [
                -5.150829928323684,
                0.0,
...
def test_energies_and_modes(n2_data, n2_vibdata):
    ...
    assert_array_almost_equal(
        n2_data["ref_frequencies"], energies / units.invcm, decimal=5
    )
    assert_array_almost_equal(
        n2_data["ref_frequencies"], n2_vibdata.get_energies() / units.invcm, decimal=5
    )
    assert_array_almost_equal(
        n2_data["ref_frequencies"], n2_vibdata.get_frequencies(), decimal=5
    )
    ...
    try:
        ...
        assert_array_almost_equal(
            atoms_with_forces.get_forces(), -n2_data["ref_forces"]
        )
...
def test_new_mass(n2_data, n2_vibdata):
    ...
    assert_array_almost_equal(
        n2_vibdata.get_energies() / np.sqrt(3), new_vib_data.get_energies()
    )
...
def test_constrained_atoms(n2_data):
    ...
    vib_data = VibrationsData(tmpAtoms, n2_data["hessian"][1:, :, 1:, :])
    assert vib_data.get_indices() == [
        1,
    ]
    assert vib_data.get_mask().tolist() == [False, True]
...
def test_todict(n2_data, n2_vibdata):
    ...
    assert_array_almost_equal(
        vib_data_dict["atoms"].positions, n2_data["atoms"].positions
    )
...
@pytest.mark.parametrize(
    "indices, expected_mask", [([1], [False, True]), (None, [True, True])]
)
def test_dict_indices(n2_vibdata, indices, expected_mask):
    ...
    if indices is not None:
        ...
        vib_data_dict["hessian"] = np.asarray(vib_data_dict["hessian"])[
            :n_active, :, :n_active, :
        ].tolist()

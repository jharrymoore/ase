...
@pytest.mark.parametrize("b", [1, 1.0])
...
def test_array_equal(a, b, rtol, atol):
    ...
    assert equal(a, b, rtol=rtol, atol=atol)
    assert equal(
        {"size": a, "gamma": True}, {"size": b, "gamma": True}, rtol=rtol, atol=atol
    )
    assert not equal(
        {"size": a, "gamma": True}, {"size": b, "gamma": False}, rtol=rtol, atol=atol
    )
...
@pytest.mark.parametrize(
    "rtol,atol", [[None, 2e-8], [0, 2e-8], [1e-8, None], [1e-8, 0], [0.5e-8, 1e-8]]
)

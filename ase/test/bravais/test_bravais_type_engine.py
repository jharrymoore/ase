import pytest
import numpy as np
from ase.geometry.bravais_type_engine import niggli_op_table, generate_niggli_op_table


def lattice_names():
    return [name for name in niggli_op_table if name not in {"MCL", "MCLC", "TRI"}]
...
def test_generate_niggli_table(lattice_name):
    ...
    table = generate_niggli_op_table(
        lattices=[lattice_name], angle_grid=angle_grid, length_grid=length_grid
    )

...
from ase.calculators.calculator import names as calculator_names, get_calculator_class
...
class AbinitFactory:
    ...
    def _base_kw(self):
        return dict(pp_paths=self.pp_paths, ecut=150, chksymbreak=0, toldfe=1e-3)
    ...
    def calc(self, **kwargs):
        ...
        if Vasp.VASP_PP_PATH not in os.environ:
            ...
            pytest.skip(
                ...
                "Set the ${} environment variable to enable.".format(Vasp.VASP_PP_PATH)
            )
...
class OpenMXFactory:
    ...
    def fromconfig(cls, config):
        return cls(
            config.executables["openmx"], data_path=config.datafiles["openmx"][0]
        )
...
class NWChemFactory:
    ...
    def version(self):
        ...
        match = re.search(
            r"Northwest Computational Chemistry Package \(NWChem\) (\S+)", stdout, re.M
        )
        return match.group(1)
...
def parametrize_calculator_tests(metafunc):
    ...
    if calculator_inputs:
        metafunc.parametrize(
            "factory", calculator_inputs, indirect=True, ids=lambda input: input[0]
        )
...
class CalculatorInputs:
    ...
    def require_version(self, version):
        ...
        if old:
            pytest.skip(
                "Version too old: Requires {}; got {}".format(

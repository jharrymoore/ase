def test_mic():
    ...
    pos = np.dot(
        np.array([[0.0, 0.0, 0.0], [0.5, 0.5, 0.5], [0.2, 0.2, 0.2], [0.25, 0.5, 0.0]]),
        cell,
    )
    ...
    assert np.all(
        abs(
            ...
            - np.array([a.get_distance(0, i, vector=True) for i in [0, 1, 2, 3]])
        )
        < tol
    )
    assert np.all(
        abs(
            ...
            - np.array([a.get_distance(1, i, vector=True) for i in [0, 1, 2, 3]])
        )
        < tol
    )
    assert np.all(
        abs(
            ...
            - np.array([a.get_distance(2, i, vector=True) for i in [0, 1, 2, 3]])
        )
        < tol
    )
    assert np.all(
        abs(
            ...
            - np.array([a.get_distance(3, i, vector=True) for i in [0, 1, 2, 3]])
        )
        < tol
    )
    ...
    assert np.all(
        abs(
            ...
            - np.array(
                [a.get_distance(0, i, mic=True, vector=True) for i in [0, 1, 2, 3]]
            )
        )
        < tol
    )
    assert np.all(
        abs(
            ...
            - np.array(
                [a.get_distance(1, i, mic=True, vector=True) for i in [0, 1, 2, 3]]
            )
        )
        < tol
    )
    assert np.all(
        abs(
            ...
            - np.array(
                [a.get_distance(2, i, mic=True, vector=True) for i in [0, 1, 2, 3]]
            )
        )
        < tol
    )
    assert np.all(
        abs(
            ...
            - np.array(
                [a.get_distance(3, i, mic=True, vector=True) for i in [0, 1, 2, 3]]
            )
        )
        < tol
    )

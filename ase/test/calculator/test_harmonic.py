...
ref_pos = np.asarray(
    [
        [8.7161, 7.96276, 8.48206],
        [8.60594, 8.04985, 9.44464],
...
def test_raise_Errors():
    with pytest.raises(CalculatorSetupError):
        HarmonicForceField(
            ref_atoms=ref_atoms, hessian_x=hessian_x, get_q_from_x=lambda x: x
        )
    with pytest.raises(CalculatorSetupError):
        HarmonicForceField(
            ref_atoms=ref_atoms, hessian_x=hessian_x, variable_orientation=True
        )
    with pytest.raises(CalculatorSetupError):
        HarmonicForceField(ref_atoms=ref_atoms, hessian_x=hessian_x, cartesian=False)
...
def test_compatible_with_ase_vibrations():
    ...
    hff = HarmonicForceField(
        ref_atoms=opt_atoms, ref_energy=opt_energy, hessian_x=hessian_2d
    )
...
def test_thermodynamic_integration():
    ...
    for lamb in lambs:
        ediffs[lamb] = []
        calc_linearCombi = MixedCalculator(
            calc_harmonic_0, calc_harmonic_1, 1 - lamb, lamb
        )
        atoms = ref_atoms.copy()
        ...
        ZeroRotation(atoms)
        with Andersen(
            atoms, 0.5 * fs, temperature_K=300, andersen_prob=0.05, fixcm=False
        ) as dyn:
    ...
    if dA == 0.0:
        raise ValueError(
            "there is most likely something wrong, but it could "

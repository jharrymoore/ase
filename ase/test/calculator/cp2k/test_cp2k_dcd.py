...
def test_dcd(factory, factories):
    ...
    if cp2k_main is None:
        pytest.skip(
            'Please define "cp2k_main" in testing executables.  '
    ...
    subprocess.check_call([cp2k_main, "-i", "test_dcd.inp", "-o", "test_dcd.out"])
    ...
    with open("test_dcd-pos-1.dcd", "rb") as fd:
        itraj = iread_cp2k_dcd(fd, indices=slice(0, None), ref_atoms=h2, aligned=True)

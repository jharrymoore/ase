...
def test_Ar_minimize(factory, ar_nc, params):
    with factory.calc(specorder=["Ar"], **params) as calc:
        ...
        assert_allclose(
            ar_nc.get_potential_energy(), -0.468147667942117, atol=1e-4, rtol=1e-4
        )
        assert_allclose(
            ar_nc.get_forces(),
        ...
        assert_allclose(
            ar_nc.get_potential_energy(), -0.4791815886953914, atol=1e-4, rtol=1e-4
        )
        assert_allclose(
            ar_nc.get_forces(),
            calc.calculate_numerical_forces(ar_nc),
...
def test_Ar_minimize_multistep(factory, ar_nc, params):
    ...
    with factory.calc(specorder=["Ar"], **params) as calc:
        ...
        assert_allclose(
            ar_nc.get_potential_energy(), -0.4791815887032201, atol=1e-4, rtol=1e-4
        )
        assert_allclose(
            ar_nc.get_forces(),
            calc.calculate_numerical_forces(ar_nc),

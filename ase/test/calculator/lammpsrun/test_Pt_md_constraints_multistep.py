...
def test_Pt_md_constraints_multistep(factory, pt_eam_potential_file):
    ...
    with factory.calc(
        specorder=["Pt"], files=[str(pt_eam_potential_file)], **params
    ) as calc:
        slab.calc = calc

        assert_allclose(
            slab.get_potential_energy(), -110.3455014595596, atol=1e-4, rtol=1e-4
        )
        ...
        assert_allclose(
            slab.get_potential_energy(), -110.4469605087525, atol=1e-4, rtol=1e-4
        )

...
def test_main(factory):
    ...
    atoms.get_forces()
    assert_allclose(
        atoms.get_potential_energy(), -2056.785854116688, rtol=1e-4, atol=1e-4
    )
    ...
    assert_allclose(
        calc2.results["energy"], atoms.get_potential_energy(), rtol=1e-4, atol=1e-4
    )
    assert_allclose(calc2.results["forces"], atoms.get_forces(), rtol=1e-4, atol=1e-4)

    # Compare analytical vs numerical forces
    assert_allclose(
        atoms.get_forces(), calc.calculate_numerical_forces(atoms), rtol=1e-4, atol=1e-4
    )

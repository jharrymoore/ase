...
def natural_cutoffs(atoms, mult=1, **kwargs):
    ...
    * kwargs: Symbol of the atom and its corresponding cutoff,
    ...
    """
    return [
        kwargs.get(atom.symbol, covalent_radii[atom.number] * mult) for atom in atoms
    ]
...
def primitive_neighbor_list(
    ...
    neigh_search_x, neigh_search_y, neigh_search_z = np.ceil(
    ...
    for c in range(3):
        if pbc[c]:
            ...
            cell_shift_ic[:, c], bin_index_ic[:, c] = divmod(
    ...
    binz_xyz, biny_xyz, binx_xyz = np.meshgrid(
        np.arange(nbins_c[2]),
        np.arange(nbins_c[1]),
        np.arange(nbins_c[0]),
        indexing="ij",
    ...
    for dz in range(-neigh_search_z, neigh_search_z + 1):
        for dy in range(-neigh_search_y, neigh_search_y + 1):
            for dx in range(-neigh_search_x, neigh_search_x + 1):
                ...
                # Shift vectors.
                _cell_shift_vector_x_n = np.resize(
                    shiftx_xyz.reshape(-1, 1),
                    (max_natoms_per_bin**2, shiftx_xyz.size),
                ).T
                _cell_shift_vector_y_n = np.resize(
                    shifty_xyz.reshape(-1, 1),
                    (max_natoms_per_bin**2, shifty_xyz.size),
                ).T
                _cell_shift_vector_z_n = np.resize(
                    shiftz_xyz.reshape(-1, 1),
                    (max_natoms_per_bin**2, shiftz_xyz.size),
                ).T
    ...
    abs_distance_vector_n = np.sqrt(
        np.sum(distance_vector_nc * distance_vector_nc, axis=1)
    )
...
class NewPrimitiveNeighborList:
    ...
    def build(self, pbc, cell, positions, numbers=None):
        ...
        pair_first, pair_second, offset_vec = primitive_neighbor_list(
            "ijS",
            pbc,
            cell,
            positions,
    ...
    def get_neighbors(self, a):
        ...
        return (
            self.pair_second[self.first_neigh[a] : self.first_neigh[a + 1]],
...
class PrimitiveNeighborList:
    ...
    def update(self, pbc, cell, coordinates):
        ...
        if (
            (self.pbc != pbc).any()
            or (self.cell != cell).any()
            or ((self.coordinates - coordinates) ** 2).sum(1).max() > self.skin**2
        ):
            self.build(pbc, cell, coordinates)
            return True
    ...
    def build(self, pbc, cell, coordinates):
        ...
        if len(self.cutoffs) != len(coordinates):
            raise ValueError(
                "Wrong number of cutoff radii: {0} != {1}".format(
        ...
        for n1, n2, n3 in itertools.product(
            range(0, N[0] + 1), range(-N[1], N[1] + 1), range(-N[2], N[2] + 1)
        ):
            ...
            for a in range(natoms):
                ...
                indices = tree.query_ball_point(
                    positions[a] - displacement, r=self.cutoffs[a] + rcmax
                )
                if not len(indices):
                    continue
...
class NeighborList:
    ...
    def __init__(
        ...
        self.nl = primitive(
            cutoffs, skin, sorted, self_interaction=self_interaction, bothways=bothways
        )

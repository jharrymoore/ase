...
def new_app(projects):
    ...
    def update(sid: int, what: str, x: str):
        ...
        * page: show page x
        ...
        table = session.create_table(
            project.database, project.uid_key, keys=list(project.key_descriptions)
        )
        return render_template(
    ...
    def row(project_name: str, uid: str):
        ...
        return render_template(
            str(project.get_row_template()), dct=dct, row=row, project=project, uid=uid
        )
    ...
    def atoms(project_name: str, id: int, type: str):
        ...
        if type == "xyz":
            ...
            con.write(row, data=row.get("data", {}), **row.get("key_value_pairs", {}))
        else:
            1 / 0

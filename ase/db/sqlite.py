...
all_tables = ["systems", "species", "keys", "text_key_values", "number_key_values"]
...
class SQLite3Database(Database):
    ...
    def _write(self, atoms, key_value_pairs, data, id):
        ...
        with self.managed_connection() as con:
            ...
            if id is None:
                ...
                cur.execute(
                    "UPDATE systems SET {} WHERE id=?".format(q), values + (id,)
                )

            count = row.count_atoms()
            if count:
                species = [
                    (atomic_numbers[symbol], n, id) for symbol, n in count.items()
                ]
    ...
    def _update(self, id, key_value_pairs, data=None):
        ...
        with self.managed_connection() as con:
            ...
            self._delete(cur, [id], ["keys", "text_key_values", "number_key_values"])
    ...
    def metadata(self, dct):
        ...
        with self.managed_connection() as con:
            cur = con.cursor()
            cur.execute("SELECT COUNT(*) FROM information WHERE name='metadata'")
            ...
            if cur.fetchone()[0]:
                ...
                cur.execute("INSERT INTO information VALUES (?, ?)", ("metadata", md))
    ...
    def _insert_in_external_table(self, cursor, name=None, entries=None):
        ...
        inserts = [
            (k, self._convert_to_recognized_types(v), id) for k, v in entries.items()
        ]

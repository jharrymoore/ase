...
def main(args):
    ...
    if args.show_values:
        ...
        for key in keys:
            vals = values[key]
            if key in numbers:
                print("{:{}} [{}..{}]".format(key + ":", n, min(vals), max(vals)))
            else:
                print(
                    "{:{}} {}".format(
                        key + ":",
    ...
    if args.insert_into:
        ...
        nkvp = 0
        nrows = 0
        with connect(args.insert_into, use_lock_file=not args.no_lock_file) as db2:
            with progressbar(
                db.select(query, sort=args.sort, limit=args.limit, offset=args.offset),
                length=length,
            ) as rows:
                for row in rows:
    ...
    if add_key_value_pairs or delete_keys:
        ...
        with db:
            for id in ids:
                m, n = db.update(id, delete_keys=delete_keys, **add_key_value_pairs)
                M += m
                N += n
        out(
            "Added %s (%s updated)"

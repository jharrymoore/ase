...
def main():
    ...
    p.add_argument("--clean", action="store_true", help="delete release branch and tag")
    args = p.parse_args()

    try:
        current_version = get_version()
    except Exception as err:
        p.error("Cannot get version: {}.  Are you in the root directory?".format(err))
...
"""
    ...
    txt, nsub = re.subn(r"ase-\d+\.\d+\.\d+", "ase-{}".format(version), txt)
    assert nsub > 0
    txt, nsub = re.subn(
        r"git clone -b \d+\.\d+\.\d+", "git clone -b {}".format(version), txt
    )
    assert nsub == 1
    ...
    git("add {}".format(" ".join([versionfile, installdoc, frontpage, releasenotes])))

...
class ColorWindow:
    ...
    def toggle(self, value):
        ...
        if value == "jmol" or value == "neighbors":
            ...
            scalars = np.ma.array(
                [self.gui.get_color_scalars(i) for i in range(len(self.gui.images))]
            )
            mn = np.min(scalars)
            mx = np.max(scalars)
            ...
            self.mnmx = [
                _("min:"),
                ui.SpinBox(
                    mn, mn - 10 * rng, mx + rng, rng / 10.0, self.change_mnmx, width=20
                ),
                _("max:"),
                ui.SpinBox(
                    mx, mn - 10 * rng, mx + rng, rng / 10.0, self.change_mnmx, width=20
                ),
                _(unit),
            ]
            self.win.close()
    ...
    def update_colormap(self, cmap=None, N=26):
        ...
        if cmap == "default":
            colorscale = [
                "#{0:02X}80{0:02X}".format(int(red)) for red in np.linspace(0, 250, N)
            ]
        elif cmap == "old":
            colorscale = [
                "#{0:02X}AA00".format(int(red)) for red in np.linspace(0, 230, N)
            ]
        else:
            try:
                import pylab as plt
                ...
                colorscale = [
                    matplotlib.colors.rgb2hex(c[:3]) for c in cmap(np.linspace(0, 1, N))
                ]
            except (ImportError, ValueError) as e:
                raise RuntimeError(
                    "Can not load colormap {0}: {1}".format(cmap, str(e))
                )
        self.gui.colormode_data = colorscale, mn, mx
        self.gui.draw()

...
class Render:
    ...
    def __init__(self, gui):
        ...
        win.add(
            [ui.Label(_("Size")), self.width_widget, ui.Label("⨯"), self.height_widget]
        )

        self.linewidth_widget = ui.SpinBox(0.07, start=0.01, end=9.99, step=0.01)
        win.add(
            [ui.Label(_("Line width")), self.linewidth_widget, ui.Label(_("Ångström"))]
        )
        ...
        self.basename_widget = ui.Entry(
            width=30, value=formula, callback=self.update_outputname
        )
        ...
        self.texture_widget = ui.ComboBox(
            labels=self.texture_list, values=self.texture_list
        )
        win.add([ui.Label(_("Atomic texture set:")), self.texture_widget])
        # complicated texture stuff

        self.camera_widget = ui.ComboBox(labels=self.cameras, values=self.cameras)
        ...
        self.frames_widget = ui.RadioButtons(
            [_("Render current frame"), _("Render all frames")]
        )
        ...
        win.add(
            [self.run_povray_widget, self.keep_files_widget, self.show_output_widget]
        )
    ...
    def ok(self, *args):
        ...
        for frame in frames:
            ...
            if self.run_povray_widget.value:
                renderer.render(
                    povray_executable=self.povray_executable.value, clean_up=False
                )

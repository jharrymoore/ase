    ...
    for i1, a in enumerate(A):
        ...
        for b in [np.zeros(3), A[i2]]:
            for c in [np.zeros(3), A[i3]]:
                ...
                p2 = p1 + a
                mlab.plot3d(
                    [p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], tube_radius=0.1
                )
...
def main(args=None):
    ...
    add("-s", "--spin-index", type=int, metavar="SPIN", help="Spin index: zero or one.")
    add(
        "-e",
    ...
    if arg.endswith(".cube"):
        ...
        if opts.band_index is None:
            ...
            data = calc.get_pseudo_wave_function(opts.band_index, opts.spin_index or 0)
    ...
    if len(contours) == 1:
        ...
        print(
            "%d contours: %.6f, ..., %.6f" % (len(contours), contours[0], contours[-1])
        )

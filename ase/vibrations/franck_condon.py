    ...
    def direct(self, n, m, S_in):
        ...
        for k in range(n + 1):
            s += (
                (-1) ** (n - k)
                * S ** float(-k)
                / (self.factorial(k) * self.factorial(n - k) * self.factorial(m - k))
            )
        res = (
            np.exp(-S) * S ** (n + m) * s**2 * (self.factorial(n) * self.factorial(m))
        )
        # use othogonality
        res[mask] = int(n == m)
        return res[0]
...
class FranckCondonRecursive:
    ...
    def direct0mm1(self, m, delta):
        ...
        if m:
            sum -= m * S ** (m - 1)
        return np.where(
            S == 0, 0, (np.exp(-S) * delta / np.sqrt(2) * sum * self.factorial.inv(m))
        )
    ...
    def get_Franck_Condon_factors(self, temperature, forces, order=1):
        ...
        for o in range(n):
            ...
            for q in range(o, n + o):
                ...
                O_n[o][q - o] = (
                    np.exp(-S / 2) / (factorial(o) * factorial(q)) ** (0.5) * summe
                ) ** 2 * w_n[o]
            for q in range(n - 1):

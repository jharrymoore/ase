...
class CLICommand:
    ...
    def add_arguments(parser):
        ...
        add("-i", "--input-format", metavar="FORMAT", help="Specify input FORMAT")
        add("output", metavar="output-file")
        add("-o", "--output-format", metavar="FORMAT", help="Specify output FORMAT")
        add("-f", "--force", action="store_true", help="Overwrite an existing file")
        add(
            "-n",
            "--image-number",
            default=":",
    ...
    def run(args, parser):
        ...
        for filename in args.input:
            atoms = read(
                filename, args.image_number, format=args.input_format, **args.read_args
            )

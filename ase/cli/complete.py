...
def match(word, *suffixes):
    return [
        w for w in glob(word + "*") if any(w.endswith(suffix) for suffix in suffixes)
    ]


# Beginning of computer generated data:
commands = {
    "band-structure": ["-o", "--output", "-r", "--range"],
    "build": [
        "-M",
        "--magnetic-moment",
        "--modify",
    ...
    "dimensionality": ["--display-all", "--no-merge"],
    "eos": ["-p", "--plot", "-t", "--type"],
    "exec": [
        "-e",
        "--exec-code",
    ...
    "find": ["-v", "--verbose", "-l", "--long", "-i", "--include", "-x", "--exclude"],
    "gui": [
        "-n",
        "--image-number",
        "-r",
    ...
    "info": ["-v", "--verbose", "--formats", "--calculators"],
    "nebplot": ["--nimages", "--share-x", "--share-y"],
    "nomad-get": [],
    "nomad-upload": ["-t", "--token", "-n", "--no-save-token", "-0", "--dry-run"],
    "reciprocal": [],
    "run": [
        "-p",
        "--parameters",
    ...
    "ulm": ["-n", "--index", "-d", "--delete", "-v", "--verbose"],
}
...
if sys.version_info[0] == 2:
    ...
    warnings.warn(
        "Command-line completion running with python2.  "
